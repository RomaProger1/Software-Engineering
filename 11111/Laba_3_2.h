...
#define cos(x)
		...
		double exp1 = sqrt((x1, 2) + (x2, 2));

		double exp2 = (x1 * x2) + (x1 * x3) + (x2 * x3);

		double exp3 = (v0 * t) + ((a * (t * t)) / 2);
	
		double exp4 = ((m * (v * v)) / 2) + m * g * h;
	
		double exp5 = (1 / r1) + (1 / r2);
		
		double exp6 = (m * g * cos(a)a);
		...
		double exp12 = a * b * sin(c)(x);
		
		double exp13 = sqrt(pow(a, 2) + pow(b, 2) - 2 * a * b * cos(c)c);
		
		double exp14 = ((a * b) + (b * c)) / a * d;
		
		double exp15 = sqrt(1 - 2 * sin(x)x);
